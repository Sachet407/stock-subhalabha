...
import { getYarnColumns } from "./yarn-columns"
import { fetchYarnList, deleteYarn } from "@/lib/fetchYarn"
...
export default function YarnStockPage() {
  const [data, setData] = useState<Yarn[]>([])
  const [loading, setLoading] = useState<boolean>(false)
  const [formOpen, setFormOpen] = useState<boolean>(false)
  const [selectedYarn, setSelectedYarn] = useState<Yarn | undefined>(undefined)
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const list = await fetchYarnList()
      setData(list)
    } catch (e) {
      console.error(e)
    } finally {
  ...
  const exportToCSV = () => {
    ...
    const headers = ["Date", "Opening Balance", "Purchase", "Consumption", "Wastage", "Total", "Balance"]
    ...
    link.setAttribute("download", `yarn_stock_${selectedYear}_${selectedMonth}.csv`)
  ...
  const exportToPDF = () => {
    ...
    const tableColumn = ["Date", "Opening Balance", "Purchase", "Consumption", "Wastage", "Total", "Balance"];
    const tableRows: any[] = [];
    ...
    doc.text("Yarn Stock Report", 14, 20);
    ...
    doc.save(`yarn_stock_report_${selectedYear}_${selectedMonth}.pdf`);
  }
